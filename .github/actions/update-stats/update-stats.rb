...
def update_project_stats (full_path, count, updated_at, pushed_at, is_archived)
  path = relativePath(full_path)
  yaml = File.read(full_path)
  obj = YAML.load(yaml)

  # TODO: do we need to be careful here
  obj.store("stats", { 'issue-count' => count, 'last-updated' => updated_at, 'last-push' => pushed_at, 'archived' => is_archived })
...
def verify_file (full_path)
  begin
    ...
    if result.data.repository.nil?
      ...
      pushed_at = result.data.repository.pushed_at
      is_archived = result.data.repository.is_archived

      # enable this at a later stage
      update_project_stats(full_path, count, updated_at, pushed_at, is_archived)
    end
